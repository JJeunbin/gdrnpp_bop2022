"""z_bracket held-out "test" split.

Unlike the real BOP challenge test sets, these scenes are BlenderProc-rendered
synthetic images with full ground-truth annotations (poses, masks) -- they're
just scenes withheld from train_pbr for internal ADD scoring. Since we have no
trained 2D detector for this object, evaluation runs with
cfg.MODEL.LOAD_DETS_TEST=False and cfg.TEST.TEST_BBOX_TYPE="gt", which means
the test-time dataloader needs real GT annotations in the dataset dicts
(unlike itodd_bop_test.py's stripped-down version) -- so this file is
essentially z_bracket_pbr.py's loader pointed at the test/ folder instead.
"""
import hashlib
import logging
import os
...
class Z_BRACKET_BOP_TEST_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/test"))
        assert osp.exists(self.dataset_root), self.dataset_root
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/z_bracket/models
        ...
        self.filter_invalid = data_cfg.get("filter_invalid", False)  # keep all GT for eval
        ##################################################

        self.cat_ids = [cat_id for cat_id, obj_name in ref.z_bracket.id2obj.items() if obj_name in self.objs]
        self.cat2label = {v: i for i, v in enumerate(self.cat_ids)}  # id_map
        self.label2cat = {label: cat for cat, label in self.cat2label.items()}
        self.obj2label = OrderedDict((obj, obj_id) for obj_id, obj in enumerate(self.objs))
        ##########################################################

        # scene folders under test/ are discovered dynamically (see z_bracket_pbr.py)
        self.scenes = sorted(
            d
            for d in os.listdir(self.dataset_root)
            if len(d) == 6 and d.isdigit() and osp.isdir(osp.join(self.dataset_root, d))
        )
    ...
    def __call__(self):
        ...
        if self.num_instances_without_valid_box > 0:
            logger.warning("Filtered out {} instances without valid box.".format(self.num_instances_without_valid_box))
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.z_bracket.obj2id[obj_name]:06d}.ply"),
...
def get_z_bracket_metadata(obj_names, ref_key):
...
SPLITS_Z_BRACKET_BOP_TEST = dict(
    z_bracket_test=dict(
        name="z_bracket_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/models"),
        objs=ref.z_bracket.objects,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_Z_BRACKET_BOP_TEST:
        used_cfg = SPLITS_Z_BRACKET_BOP_TEST[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, Z_BRACKET_BOP_TEST_Dataset(used_cfg))
    MetadataCatalog.get(name).set(
        ...
        evaluator_type="bop",
        **get_z_bracket_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_Z_BRACKET_BOP_TEST.keys())
