...
class Z_BRACKET_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/train_pbr"))
        ...
        # NOTE: unlike the original BOP-challenge PBR sets (fixed 50 scenes),
        # this dataset was generated incrementally with BlenderProc, so the
        # number of scene folders may grow over time -> discover them dynamically.
        self.scenes = sorted(
            d
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    # assert osp.exists(xyz_path), xyz_path  # NOTE: not needed, XYZ_ONLINE=True renders xyz on the fly
                    inst = {
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.z_bracket.obj2id[obj_name]:06d}.ply",
                ),
...
SPLITS_Z_BRACKET_PBR = dict(
    z_bracket_train_pbr=dict(
        name="z_bracket_train_pbr",
        objs=ref.z_bracket.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/z_bracket/train_pbr/xyz_crop"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_Z_BRACKET_PBR:
        used_cfg = SPLITS_Z_BRACKET_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, Z_BRACKET_PBR_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_Z_BRACKET_PBR.keys())
